...
func _physics_process(delta):
	...
	if (spawnTimer >= spawnInteval):
		...
		if (Globals.bossMode == false and monsters.get_child_count() < max_monsters):
			var monsterType
			#if level_borders.position.x <= bossDistances[0]:
			monsterType = random.randi_range(0, 60)
			#if level_borders.position.x > bossDistances[0]:
			#	monsterType = random.randi_range(61, 120)
			
			if (monsterType < 40 or (monsterType >= 61 and monsterType < 80)):
				spawnWal()
				
			if ((monsterType >= 40 and monsterType < 60) or (monsterType >= 80 and monsterType < 90)):
				spawnStab()
				
			if (monsterType >= 90 and monsterType < 120):
				spawnKamel()
				
			if (monsterType == 60 || monsterType == 120):
				spawnGeflügelteGekrönteGoldeneErdbeere()
	
	#spawn bosses                   30000
	if (level_borders.position.x >= bossDistances[0] and level_borders.position.x <= bossDistances[0] + 200 and !Globals.giraffeBeaten and !Globals.bossMode):
...
func spawnKamel():
	var position = Vector2((level_borders.position.x + camera.get_viewport_rect().size.x / camera.zoom.x + 100), 1080)
	var kamel = kamel_template.duplicate()
	kamel.position = position
	monsters.add_child(kamel)

func spawnGeflügelteGekrönteGoldeneErdbeere():
